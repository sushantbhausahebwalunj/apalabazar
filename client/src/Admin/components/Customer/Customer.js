...
import { fetchUsers, deleteUser } from '../../../Redux/User/userSlice'; // Adjust the import path as needed
    ...
    <div className="min-h-screen bg-zinc-100 p-4">
      <div className="container mx-auto bg-white rounded-lg shadow-md p-6">
              ...
              {currentUsers.map((user) => (
                <tr key={user._id} className="hover:bg-zinc-50 transition duration-200">
                  <td className="py-2 px-4 border-b flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-medium">{user.firstName} {user.lastName}</p> {/* Updated to show firstName and lastName */}
                      <p className="text-zinc-500 text-sm">{user.userName}</p> {/* Updated to show user role */}
                    </div>
                  </td>
        ...
        <div className="flex justify-between items-center mt-8">
          <p className="text-zinc-600 text-xs">Showing {currentUsers.length} of {users.length} entries</p>
